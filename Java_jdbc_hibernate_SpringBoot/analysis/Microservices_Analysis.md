...
4. Decentralized Governance: Each team picks own tech stack
...
public RouteLocator routes(RouteLocatorBuilder builder) {
    return builder.routes()
        .route("policy-service", r -> r
            ...
            .filters(f -> f.circuitBreaker(c -> c.setName("policyCB")))
...
4. Fallback: Return cached/default response
5. Bulkhead: Isolate thread pools per service call
6. Health checks: /actuator/health for auto-restart (K8s)
7. Dead Letter Queue: Failed messages go to DLQ for manual processing
...
Service A -> Envoy Proxy A -> Network -> Envoy Proxy B -> Service B
...
2. Use polling or WebSocket for status update
