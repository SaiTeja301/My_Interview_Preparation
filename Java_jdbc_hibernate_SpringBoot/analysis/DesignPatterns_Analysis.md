...
CGLIB Proxy: For classes (creates subclass at runtime)
...
3. Builder: DTO construction (Lombok @Builder)
4. Strategy: Payment processing (different payment modes)
5. Observer: Spring Events for order workflows
6. Proxy: Spring AOP (@Transactional, @Cacheable)
...
// Old payment system
class OldPaymentGateway { String makePayment(String xml) {...} }

// New system expects:
interface PaymentProcessor { Response process(PaymentRequest req); }
