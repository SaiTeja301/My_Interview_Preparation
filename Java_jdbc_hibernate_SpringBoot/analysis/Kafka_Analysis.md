...
- Partition: Physical division of topic (parallelism + ordering)
...
Setup steps (from your notes):
1. Generate Keystore: keytool -genkey -keyalg RSA -keystore kafka.server.keystore.jks
2. Create CA: openssl req -new -x509 -keyout ca-key -out ca-cert -days 365
3. Export cert, sign with CA, import to keystore
...
Producer acks:
acks=0: Don't wait for any ack (fastest, may lose)
acks=1: Wait for leader ack (balanced)
acks=all: Wait for all ISR acks (safest, slowest)
...
Alerting: Set up alerts when lag exceeds threshold
