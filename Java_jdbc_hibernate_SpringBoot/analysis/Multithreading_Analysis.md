...
// Callable
Callable<Integer> calc = () -> { return 42; };
...
Use for: boolean flags, singleton double-check locking
Do NOT use for: compound operations (i++, check-then-act)
...
- workQueue: Queue for pending tasks (ArrayBlockingQueue, LinkedBlockingQueue)
- rejectionPolicy: What to do when queue full
...
4. Use concurrent utilities instead of raw synchronization

Code (Prevention with lock ordering):
